...
class TitanicLogRegBaseline(Optimizable):
    ...
    DEFAULT_HPARAMS: Hyperparameters = {
        ...
        # Base model hyperparameters (used when use_grid_search=False)
        'C': 1.0,
        'penalty': 'l2',
        'class_weight': None,
    ...
    def run(self, hparams: Hyperparameters) -> Metrics:
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            X,
            y,
            test_size=hparams.get('test_size', 0.2),
            stratify=y,
        ...
        # Training with or without grid search
        if hparams.get('use_grid_search', True):
            param_grid = {
                'model__C': hparams.get('grid_C', [0.1, 1.0, 10.0]),
                'model__penalty': hparams.get('grid_penalty', ['l2', 'l1']),
                'model__class_weight': hparams.get('grid_class_weight', [None, 'balanced']),
            }
            search = GridSearchCV(
                estimator=pipeline,
                param_grid=param_grid,
                scoring=hparams.get('scoring', 'roc_auc'),
                cv=hparams.get('cv', 5),
            ...
            pipeline.set_params(
                **{
                    'model__C': hparams.get('C', 1.0),
                    'model__penalty': hparams.get('penalty', 'l2'),
                    'model__class_weight': hparams.get('class_weight', None),
                }
            )
